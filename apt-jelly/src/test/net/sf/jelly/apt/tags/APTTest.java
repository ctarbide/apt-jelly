...
import net.sf.jelly.apt.APTJellyProcessorFactory;
...
/**
 * Base test for apt stuff.
 * <p/>
 *
 * @author Ryan Heaton
 */
public abstract class APTTest extends TestCase {
  ...
  public void runScript(URL url) {
    ...
    int procCode = com.sun.tools.apt.Main.process(new APTTestProcessorFactory(url), aptOpts.toArray(new String[aptOpts.size()]));
  ...
  /**
   * An APTJellyProcessorFactory that can be invoked more than once in the same JVM.
   */
  private class APTTestProcessorFactory extends APTJellyProcessorFactory {

    public APTTestProcessorFactory(URL script) {
      super(script);
